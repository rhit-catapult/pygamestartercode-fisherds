    ...
    *** See the PDFs, especially
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   03a-dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        h_rect_up_left = rg.Point(intersection_center.x - width / 2, intersection_center.y - letter_thickness / 2)
        h_rect_low_right = rg.Point(intersection_center.x + width / 2, intersection_center.y + letter_thickness / 2)

        v_rect_up_left = rg.Point(intersection_center.x - letter_thickness / 2, intersection_center.y - letter_thickness / 2)
        v_rect_low_right = rg.Point(intersection_center.x + letter_thickness / 2, intersection_center.y + height - letter_thickness / 2)

        self.h_rect = rg.Rectangle(h_rect_up_left, h_rect_low_right)
        self.v_rect = rg.Rectangle(v_rect_up_left, v_rect_low_right)
    ...
    def clone(self):
        ...
        #######################################################################
        int_center = self.h_rect.get_center().clone()
        width = self.h_rect.get_width()
        height = self.v_rect.get_height()
        let_thick = self.v_rect.get_width()
        new_t = CapitalT(int_center, width, height, let_thick)
